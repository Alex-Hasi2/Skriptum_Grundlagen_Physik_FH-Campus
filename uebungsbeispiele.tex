...
\begin{center}
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}\label{fig: integral_func_aufg}
...
\begin{enumerate}
    ...
    \begin{center}
        ...
        \begin{tikzpicture}
            ...
            \end{axis}
            \end{tikzpicture}\label{fig: ober_unter_summe}
    \end{center}
...
\begin{alignat*}{2}
    ...
    \text{6.} \quad & \SI{101325}{\pascal} &&=\quad \underline{\hspace{1.5cm}} \;\si{\barPr} \\
...
\begin{enumerate}
    ...
    \item Die Definition von Bar ist $\SI{1}{\barPr} = \SI{e5}{\pascal}$.
    \begin{equation}
        \SI{101325}{\pascal} = \frac{\num{101325}}{\num{e5}}\,\si{\barPr} = \SI{1.01325}{\barPr}
    \end{equation}
...
\begin{enumerate}
    ...
    \item Die Graphen für $r_{x}(t)$ und $v_{x}(t)$ sind gegeben. Bestimmen Sie die Steigung der Tangente von $r_x(t)$ zu den Zeitpunkten $t \in \{1,0;\, 2,0;\, 3,5\}\,\si{\second}$ und überprüfen Sie Ihre Werte anhand der Kurve $v_{x}(t)$.\newline
    \textit{Hinweis:} 
    \begin{equation}
    ...
    \begin{center}
            ...
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}\label{fig: fadenpendel_rx_vx_von_t}
    \end{center}
...
\begin{enumerate}
    ...
    $F_{G,x} =\text{?}$, $F_{G,z} =\text{?}$, $F_{S,x} =\text{?}$, $F_{S,z} =\text{?}$.
...
\begin{enumerate}
    ...
    \item Das System ruht, solange die wirkende Haftreibungskraft nicht die maximale Haftreibungskraft überschreitet: $F_{R,h} \le F_{R,h,\text{max}} = \mu_{R,h} |\ivecS{F}{N}|$. Ersetzen Sie in dieser Ungleichung die Haftreibungskraft $F_{R,h}$ und die Normalkraft $|\ivecS{F}{N}|$ durch die entsprechenden Ausdrücke aus Ihren vorherigen Überlegungen. 
...
\begin{aufgabebox}{Kugel in Block mit Feder}{kugel_block_muendung}
Eine Kugel der Masse $m$ fliegt mit der gesuchten Mündungsgeschwindigkeit $v$ horizontal auf einen anfangs ruhenden Block der Masse $M$ zu. Der Block ist auf einer reibungsfreien horizontalen Unterlage gelagert und mit einer Feder der Federkonstante $k$ verbunden. Die Kugel bleibt beim Stoß im Block stecken, sodass sich beide Körper danach gemeinsam weiterbewegen. Die Feder wird dabei maximal um
\begin{equation*}
    x_{\text{max}} = \SI{9.460}{\centi\meter}
\end{equation*}
komprimiert.

\begin{center}
    \includegraphics[width=0.78\textwidth]{Bilder/Uebungsaufgaben/KugelBlockMuendungsgeschwindigkeit.png}
\end{center}

Verwenden Sie für die numerische Auswertung am Ende die Werte
\begin{align*}
    m &= \SI{20}{\gram} \mComma \\
    M &= \SI{2.00}{\kilo\gram} \mComma \\
    k &= \SI{800}{\newton\per\meter} \mDot
\end{align*}

\begin{enumerate}
    \item Stellen Sie die Impulserhaltung für den kurzen Stoßprozess auf und bestimmen Sie daraus die gemeinsame Geschwindigkeit $u$ von Kugel und Block direkt nach dem Stoß.
    \item Verwenden Sie für die anschließende Kompression der Feder den Energieerhaltungssatz und stellen Sie einen Zusammenhang zwischen $u$, $k$, $M$, $m$ und $x_{\text{max}}$ her.
    \item Bestimmen Sie daraus die Mündungsgeschwindigkeit $v$ der Kugel in Abhängigkeit von $m$, $M$, $k$ und $x_{\text{max}}$. Setzen Sie erst ganz am Ende $x_{\text{max}} = \SI{9.460}{\centi\meter}$ ein.
    \item Warum darf man den Energieerhaltungssatz nicht unmittelbar auf den Stoß selbst anwenden?
\end{enumerate}
\end{aufgabebox}
...
\end{loesungbox}
    ...
    \begin{loesungbox}{Lösung zu \Cref{aufg:kugel_block_muendung}}
    \begin{enumerate}
        \item \textbf{Impulserhaltung beim Stoß:} \\
        Während des sehr kurzen Stoßes ist der äußere Impulsübertrag in horizontaler Richtung vernachlässigbar. Daher ist der Gesamtimpuls von Kugel und Block erhalten. Mit $u$ als gemeinsamer Geschwindigkeit direkt nach dem Stoß gilt
        \begin{equation}\label{eq: kugel_block_impuls}
            m v = (M+m)u \mDot
        \end{equation}
        Aufgelöst nach $u$ ergibt das
        \begin{equation}\label{eq: kugel_block_u}
            u = \frac{m}{M+m}v \mDot
        \end{equation}

        \item \textbf{Energieerhaltung bei der Federkompression:} \\
        Nach dem Stoß bewegen sich Kugel und Block gemeinsam. Von diesem Zeitpunkt an wird die kinetische Energie des Systems vollständig in potenzielle Energie der Feder umgewandelt. Am Punkt maximaler Kompression ist die Geschwindigkeit null. Daher gilt
        \begin{equation}\label{eq: kugel_block_energie}
            \frac{1}{2}(M+m)u^2 = \frac{1}{2}k x_{\text{max}}^2 \mDot
        \end{equation}
        Daraus folgt für die Geschwindigkeit direkt nach dem Stoß
        \begin{equation}\label{eq: kugel_block_u_energie}
            u = x_{\text{max}}\sqrt{\frac{k}{M+m}} \mDot
        \end{equation}

        \item \textbf{Mündungsgeschwindigkeit der Kugel:} \\
        Wir setzen \Cref{eq: kugel_block_u_energie} in \Cref{eq: kugel_block_impuls} ein:
        \begin{align}
            m v &= (M+m) x_{\text{max}}\sqrt{\frac{k}{M+m}} \\
            &= x_{\text{max}}\sqrt{k(M+m)} \mDot
        \end{align}
        Damit erhält man für die gesuchte Mündungsgeschwindigkeit
        \begin{equation}\label{eq: kugel_block_muendung_v}
            v = \frac{x_{\text{max}}}{m}\sqrt{k(M+m)} \mDot
        \end{equation}
        Mit
        \begin{equation}
            x_{\text{max}} = \SI{9.460}{\centi\meter} = \SI{0.09460}{\meter}
        \end{equation}
        folgt auch die praktische Schreibweise
        \begin{equation}
            v = \frac{\SI{0.09460}{\meter}}{m}\sqrt{k(M+m)} \mDot
        \end{equation}
        Für die numerische Auswertung setzen wir nun erst ganz am Ende
        \begin{equation*}
            m = \SI{0.020}{\kilo\gram} \mComma \qquad M = \SI{2.00}{\kilo\gram} \mComma \qquad k = \SI{800}{\newton\per\meter}
        \end{equation*}
        ein und erhalten
        \begin{align}
            v &= \frac{\SI{0.09460}{\meter}}{\SI{0.020}{\kilo\gram}}\sqrt{\SI{800}{\newton\per\meter}\cdot\left(\SI{2.00}{\kilo\gram}+\SI{0.020}{\kilo\gram}\right)} \\
            &\approx \SI{190.1}{\meter\per\second} \approx \SI{684}{\kilo\meter\per\hour} \mDot
        \end{align}

        \item \textbf{Warum nicht Energieerhaltung im Stoß?} \\
        Beim Stoß handelt es sich um einen vollkommen unelastischen Stoß, da die Kugel im Block stecken bleibt. Dabei wird ein Teil der anfänglichen kinetischen Energie in innere Energie, Wärme und Deformationsarbeit umgewandelt. Deshalb ist die mechanische Energie während des Stoßes nicht erhalten. Erhalten bleibt in horizontaler Richtung jedoch der Impuls. Erst nach dem Stoß kann für das System aus Block, Kugel und Feder wieder die mechanische Energieerhaltung verwendet werden.
    \end{enumerate}
    \end{loesungbox}
...
Ein Autoreifen wird nach einer längeren Fahrt an einer Tankstelle bis zu einem Druck von \SI{2.2}{\barPr} bei einer Reifentemperatur von \SI{40}{\degreeCelsius} mit Luft gefüllt. Am nächsten Tag ist die Außentemperatur auf \SI{10}{\degreeCelsius} gesunken, und die Reifen haben sich entsprechend abgekühlt.
\begin{center}
...
\begin{enumerate}
    ...
    \item Wie viele Mol Luft befinden sich in einem Autoreifen mit einem Volumen von $V = \SI{40}{\litre}$ bei den ursprünglichen \SI{2.2}{\barPr} und \SI{40}{\degreeCelsius}? (Gegeben: $R = \SI{8.314}{\joule\per(\mole\cdot\kelvin)}$)
    \item Skizzieren Sie den typischen Betriebsbereich eines Autoreifens (Drücke um 2 bis 3 \si{\barPr}, Temperaturen von ca. -20 bis 50\si{\celsius}) qualitativ in dem Phasendiagramm für Luft ein. Ist unter normalen Bedingungen eine Verflüssigung der Luft im Reifen zu befürchten?
\end{enumerate}
\end{aufgabebox}
...
\begin{aufgabebox}{Zylinder unter Wasser}{zylinder_unter_wasser}
...
\begin{center}
    \includegraphics[width=0.36\textwidth]{Bilder/Uebungsaufgaben/ZylinderUnterwasser.png}
\end{center}
...
\begin{enumerate}
    ...
    \begin{equation}\label{eq: druck_reifen_neu}
        p_2 = p_1 \cdot \frac{T_2}{T_1} = \SI{2.2}{\barPr} \cdot \frac{\SI{283.15}{\kelvin}}{\SI{313.15}{\kelvin}} \approx \SI{1.99}{\barPr}
    \end{equation}
    Der Druck im Reifen beträgt bei \SI{10}{\degreeCelsius} ca. \SI{1.99}{\barPr}.
    ...
    \begin{itemize}
        \item $p_1 = \SI{2.2}{\barPr} = \SI{2.2e5}{\pascal}$
    ...
    Der typische Betriebsbereich eines Autoreifens liegt bei Drücken um \SIrange{2}{3}{\barPr} und Temperaturen von ca. \SIrange{-20}{50}{\celsius}. Dieser Bereich befindet sich im Phasendiagramm von Luft (hauptsächlich Stickstoff und Sauerstoff) weit oberhalb der Siedetemperatur und weit entfernt von der Sublimations- oder Schmelzkurve. Eine Verflüssigung oder Erstarrung ist unter normalen Fahrbedingungen ausgeschlossen.
    \begin{center}
...
\begin{loesungbox}{Lösung zu \Cref{aufg:zylinder_unter_wasser}}
\begin{enumerate}
    \item \textbf{Kräfte und Gleichgewicht:}
    Auf den vollständig eingetauchten Zylinder wirken drei Kräfte:
    \begin{itemize}
        \item die Gewichtskraft $\ivecS{F}{G}$ nach unten,
        \item die Auftriebskraft $\ivecS{F}{A}$ nach oben,
        \item die Federkraft $\ivecS{F}{F}$ des Federkraftmessers nach oben.
    \end{itemize}
    Wählen wir die positive Richtung nach oben, so lautet die Gleichgewichtsbedingung:
    \begin{equation}\label{eq: zylinder_unter_wasser_gleichgewicht}
        \sum F_y = F_{\text{F}} + F_A - F_G = 0 \mDot
    \end{equation}

    \item \textbf{Volumen, Masse und Gewichtskraft:}
    Der Zylinder hat den Radius
    \begin{equation}
        r = \frac{d}{2} = \frac{\SI{8.0}{\centi\metre}}{2} = \SI{4.0}{\centi\metre} = \SI{0.040}{\metre} \mDot
    \end{equation}
    Sein Volumen ist damit
    \begin{equation}\label{eq: zylinder_unter_wasser_volumen}
        V = \pi r^2 h \mDot
    \end{equation}
    Die Masse des Aluminiumzylinders folgt zu
    \begin{equation}\label{eq: zylinder_unter_wasser_masse}
        m = \rho_{\text{Al}} V = \rho_{\text{Al}} \pi r^2 h \mDot
    \end{equation}
    Damit ergibt sich für die Gewichtskraft
    \begin{equation}\label{eq: zylinder_unter_wasser_gewichtskraft}
        F_G = m g = \rho_{\text{Al}} \pi r^2 h g \mDot
    \end{equation}

    \item \textbf{Auftriebskraft:}
    Nach dem Archimedischen Prinzip ist die Auftriebskraft gleich der Gewichtskraft des verdrängten Wassers. Da der Zylinder vollständig eingetaucht ist, ist das verdrängte Volumen gleich dem Zylindervolumen:
    \begin{equation}\label{eq: zylinder_unter_wasser_auftrieb}
        F_A = \rho_{\text{W}} V g = \rho_{\text{W}} \pi r^2 h g \mDot
    \end{equation}

    \item \textbf{Formel für die Federkraft:}
    Aus \Cref{eq: zylinder_unter_wasser_gleichgewicht} folgt unmittelbar
    \begin{equation}\label{eq: zylinder_unter_wasser_federkraft}
        F_{\text{F}} = F_G - F_A \mDot
    \end{equation}
    Einsetzen von \Cref{eq: zylinder_unter_wasser_gewichtskraft,eq: zylinder_unter_wasser_auftrieb} liefert
    \begin{equation}
        F_{\text{F}} = (\rho_{\text{Al}} - \rho_{\text{W}}) \pi r^2 h g \mDot
    \end{equation}

    \item \textbf{Numerische Anzeige des Federkraftmessers:}
    Zuerst berechnen wir das Volumen:
    \begin{align}
        V &= \pi (\SI{0.040}{\metre})^2 \cdot \SI{0.120}{\metre} \\
        &\approx \SI{6.03e-4}{\metre\cubed} \mDot
    \end{align}
    Daraus folgen Masse und Gewichtskraft des Zylinders:
    \begin{align}
        m &= \SI{2700}{\kilogram\per\metre\cubed} \cdot \SI{6.03e-4}{\metre\cubed} \approx \SI{1.63}{\kilogram} \\
        F_G &= m g \approx \SI{1.63}{\kilogram} \cdot \SI{9.81}{\metre\per\second\squared} \approx \SI{15.97}{\newton} \mDot
    \end{align}
    Die Auftriebskraft ist
    \begin{align}
        F_A &= \SI{1000}{\kilogram\per\metre\cubed} \cdot \SI{6.03e-4}{\metre\cubed} \cdot \SI{9.81}{\metre\per\second\squared} \\
        &\approx \SI{5.92}{\newton} \mDot
    \end{align}
    Damit ergibt sich für die Federkraft beziehungsweise die Anzeige des Federkraftmessers
    \begin{equation}
        F_{\text{F}} = F_G - F_A \approx \SI{15.97}{\newton} - \SI{5.92}{\newton} \approx \SI{10.06}{\newton} \mDot
    \end{equation}
    Mit unserer Vorzeichenkonvention (nach oben positiv) ist die Anzeige also
    \begin{equation}
...
\begin{enumerate}
    ...
    \begin{equation} \label{eq: dichte_gas_1}
    ...
    \end{equation}
    Wir setzen die Dichteformel \Cref{eq: dichte_gas_1} für $\rho_{\text{Innen,min}}$ ein:
    \begin{equation}
...
\begin{enumerate}
    \item Wie groß ist der Impulsübertrag $\Delta \ivecS{P}{(1)}$ des Teilchens beim Stoß mit der Wand bei der Geschwindigkeit $\ivec{v}$?
    ...
    \item Die Kraft für 1 Teilchen ist dann $\ivecS{F}{(1)}=\Delta \ivecS{P}{(1)}/\Delta t$ und der Druck ist $p_{(1)}=|\ivecS{F}{(1)}|/A$. Welchen Druck übt also 1 Teilchen auf die Innenfläche der Kugel aus?
...
\begin{enumerate}
    ...
    \item Berechnen Sie das Volumen von einem Mol eines idealen Gases unter Normalbedingungen ($p=\SI{1.013}{\barPr}$, $T=\SI{0}{\degreeCelsius}$)!
\end{enumerate}
\end{aufgabebox}
...
\begin{itemize}
    \item \textbf{A $\rightarrow$ B:} Der Zyklus beginnt bei Zustand A ($V_A = \SI{1}{\litre}$, $p_A = \SI{2}{\barPr}$) mit einer isobaren Expansion (konstanter Druck) bis $V_B = \SI{2.5}{\litre}$.
    \item \textbf{B $\rightarrow$ C:} Anschließend wird das Gas bei konstantem Volumen (isochor) abgekühlt, bis ein Druck von $p_C = \SI{1}{\barPr}$ erreicht ist.
    \item \textbf{C $\rightarrow$ D:} Nun wird das Gas bei konstantem Druck (isobar) auf ein Volumen von $V_D = \SI{1}{\litre}$ komprimiert.
    \item \textbf{D $\rightarrow$ A:} Schließlich wird das Gas bei konstantem Volumen (isochor) erwärmt, bis der Anfangszustand mit $p_A = \SI{2}{\barPr}$ wieder erreicht ist.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        Der Druck ist konstant bei $p_A = \SI{2}{\barPr} = \SI{2e5}{\pascal}$. Das Volumen ändert sich von $V_A = \SI{1}{\litre} = \SI{e-3}{\meter\cubed}$ auf $V_B = \SI{2.5}{\litre} = \SI{2.5e-3}{\meter\cubed}$.
        \begin{multline}
        ...
        Der Druck ist konstant bei $p_C = \SI{1}{\barPr} = \SI{1e5}{\pascal}$. Das Volumen ändert sich von $V_C = \SI{2.5}{\litre}$ auf $V_D = \SI{1}{\litre}$.
        \begin{multline}
