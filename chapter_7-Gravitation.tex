...
\begin{figure}[htb]
    ...
    \caption{Eine elliptische Planetenbahn um die Sonne, die in einem der beiden Brennpunkte steht. Dargestellt sind die große Halbachse $a$, die kleine Halbachse $b$ sowie Perihel, Aphel und der Ortsvektor $\protect\ivec{r}$ (Fahrstrahl) eines Planeten.}
    \label{fig: kepler1_ellipse}
\end{figure}

\subsection{Zweites Keplersches Gesetz}
\label{ssec:kepler2}
...
\begin{figure}[htb]
    ...
    \caption{Veranschaulichung des zweiten Keplerschen Gesetzes. In gleichen Zeitintervallen $\Delta t$ überstreicht der Fahrstrahl $\protect\ivec{r}(t)$ gleiche Flächen ($A_1 = A_2$).}
    \label{fig: kepler2_flaechensatz}
\end{figure}
...
Das zweite Keplersche Gesetz ist eine direkte Konsequenz der \textbf{Drehimpulserhaltung} für eine Zentralkraft. Betrachten wir ein infinitesimales Zeitintervall $\dd t$, in dem der Planet eine Strecke $\dd\ivec{s} = |\ivec{v}| \dd t$ zurücklegt, dargestellt in \cref{fig: kepler2_herleitung}. Die dabei überstrichene Fläche $\dd A$ kann durch die Fläche eines Dreiecks mit den Eckpunkten ($S, P_1, P_2$) angenähert werden. Die Seitenlängen betragen $\ivec{r}(t)$, $\ivec{r}(t+\dd t)$ und $\dd\ivec{r}$, wobei $\dd \ivec{r} = \ivec{r}(t+\dd t) - \ivec{r}(t)$. Wir wissen ja bereits aus \cref{subsubsec: herleitung_bogenlänge_sehnenlänge}, dass $\lim_{\Delta \varphi \to 0} |\Delta \ivec{r}| = \Delta s$ (\cref{eq: sehnenlänge_gleich_bogenlänge}), weshalb diese Näherung für $\dd t \to 0$ gegen die Fläche $\dd A$ konvergiert.
\begin{figure}[htb]
    \centering
    ...
    \caption{Das infinitesimale Flächenelement $\dd A$, das vom Fahrstrahl $\protect\ivec{r}(t)$ in der Zeit $\dd t$ überstrichen wird.}
    \label{fig: kepler2_herleitung}
\end{figure}
...
\begin{equation}
    ...
    \label{eq:flaechengeschwindigkeit_drehimpuls}
\end{equation}
Da das zweite Keplersche Gesetz besagt, dass die Flächengeschwindigkeit konstant ist ($\dd A/\dd t = \const$), folgt daraus direkt, dass auch der Betrag des Drehimpulses $|\ivec{L}|$ konstant sein muss. Dies ist immer der Fall, wenn die wirkende Kraft (hier die Gravitationskraft) eine Zentralkraft ist, d.h. immer auf das Zentrum (die Sonne) gerichtet ist.




\subsection{Drittes Keplersches Gesetz}
\label{ssec:kepler3}
...
\begin{figure}[tb]
    ...
    \caption{Ein Planet, der eine Kreisbahn um die Sonne ausführt.}
    \label{fig: planet_kreisbahn}
\end{figure}
...
\begin{equation}\label{eq: grav_vs_zentri}\begin{gathered}
    F_\text{G} = F_\text{Zp} \\
...
\end{equation}
Setzt man diesen Ausdruck für $\omega^2$ in Gleichung \ref{eq: grav_vs_zentri} ein, erhält man:
\begin{equation}
...
\begin{figure}[htb]
    ...
    \caption{Ein Objekt wird von der Oberfläche eines Himmelskörpers (Masse $M$, Radius $R$) mit der Anfangsgeschwindigkeit $\protect\ivec{v}_0$ senkrecht nach oben abgefeuert.}
    \label{fig: zweite_kosmische_geschwindigkeit}
\end{figure}
...
\end{equation}
Um die maximale Höhe $h_{\text{max}}$ zu finden, die bei einer Anfangsgeschwindigkeit $v_0$ erreicht wird, ersetzen wir $r_{\text{f}}$ durch $r_{\text{f}} = R + h_{max}$. Für den Umkehrpunkt gilt außerdem $v_{\text{f}} = 0$: 
\begin{equation}
    0 = v_0^2 - 2GM \left( \frac{1}{R} - \frac{1}{R+h_{max}} \right) = v_0^2 - 2GM \frac{h_{max}}{R(R+h_{max})} \mDot
\end{equation}
Mit der Erdbeschleunigung an der Oberfläche, $g = GM/R^2$, lässt sich dies umformen zu
\begin{equation}\label{eq: hmax}
    v_0^2 = \frac{2gR h_{max}}{R+h_{max}} \quad \implies \quad h_{max} = \frac{v_0^2 R}{2gR - v_0^2} \mDot
\end{equation}
Aus dieser Gleichung wird ersichtlich, dass die maximale Höhe $h_{max}$ unendlich wird, wenn der Nenner gegen Null geht. Dies geschieht, wenn die Anfangsgeschwindigkeit $v_0$ einen kritischen Wert erreicht.

\begin{importantbox}{Zweite kosmische Geschwindigkeit $v_{II}$}
Die Fluchtgeschwindigkeit ist die minimale Anfangsgeschwindigkeit, die ein Objekt benötigt, um das Gravitationsfeld eines Himmelskörpers ohne weiteren Antrieb zu verlassen ($h_{max} \to \infty$). Man erhält sie, indem man den Nenner in Gleichung \ref{eq: hmax} gleich null setzt:
\begin{equation}
