...
\end{figure}
Die \textbf{Druckabnahme} einer isothermen Lufthülle folgt also einem \textbf{Exponentialgesetz} im Gegensatz zur \textbf{linearen Druckabnahme in einer Flüssigkeit}. Die Lufthülle der Erde hat also keine scharfe Grenze. In der realen Erdatmosphäre ändert sich die Temperatur mit der Höhe. Die barometrische Höhenformel ist für die untere Atmosphäre eine gute Näherung. 
\begin{figure} % [H] erzwingt Position "genau hier"
    \centering
    \includegraphics[width=0.7\textwidth]{Bilder/Kapitel_Waermelehre/hoehenformel.pdf}
    \caption{Der exponentiell abnehmende Luftdruck $p(z)$ über der Höhe $h$ in km.}\label{fig: barom_hoehenformel_grafik}
\end{figure}
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}[>=Latex] % Setzt den Standard-Pfeilspitzen-Typ
        ...
        \def\cylW{3.0}   % Breite des Zylinders
        \def\cylH{3.8}   % Höhe der Wände
        \def\pistY{2.6}  % y-Position des Kolbenbodens
    ...
    \caption{Durch einen beweglichen Kolben kann das Volumen $V$ und der Druck $p$ eines Gases verändert werden, wobei der äußere Druck $p_\atm$ konstant bleibt.}\label{fig: kolben_zylinder_gas_pV}
\end{figure}
