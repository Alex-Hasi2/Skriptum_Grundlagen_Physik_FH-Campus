...
Hierzu betrachten wir ein kleines Volumenelement $V_S$ vor der Wand, das alle Teilchen enthält, die in der Zeit $\Delta t$ die Wand erreichen können (siehe \cref{fig: teilchen_in_box}). Die Seitenlänge dieses Quaders in $x$-Richtung ist $\dd s = v_x \cdot \dd t$, da nur Teilchen mit der Geschwindigkeitskomponente $v_x$ die Wand in dieser Zeit erreichen können. Die anderen Seitenlängen sind so gewählt, dass sie alle Teilchen mit dieser $v_x$-Komponente umfassen.
