...
\begin{enumerate}
    ...
    Die rms(\textit{root-mean-square})-Geschwindigkeit ist entscheidend für die kinetische Energie ($\overline{E_{\text{kin}}} \propto \overline{v^2}$). Sie ist etwas größer als die mittlere Geschwindigkeit, da schnelle Teilchen quadratisch stärker ins Gewicht fallen.
    \begin{equation}
...
Bisher haben wir gesehen, dass die Temperatur ein Maß für die mikroskopische Bewegungsenergie der Teilchen ist ($T \propto \overline{E_{\text{kin}}}$). 
...
Ein Mol enthält exakt $N_A \approx \SI{6.022e23}{}$ Teilchen (Avogadro-Konstante). Die ideale Gasgleichung lässt sich damit in ihrer molaren Form schreiben:
...
Aus der kinetischen Gastheorie wissen wir, dass pro Freiheitsgrad $f$ eine mittlere Energie von $\frac{1}{2}k_B T$ gespeichert wird (Gleichverteilungssatz).
Für ein Gas mit $f$ Freiheitsgraden gilt daher:
\begin{equation}
    U = N \cdot \overline{E_\mathrm{kin}} = N \cdot \frac{f}{2} k_B T = v \cdot N_A \cdot \frac{f}{2} k_B T \mDot
\end{equation}
Da $N_A \cdot k_B = R$, folgt der fundamentale Zusammenhang:
\begin{equation}\label{eq:innere_energie_f}
    U(T) = \frac{f}{2} \cdot v \cdot R \cdot T \mDot
\end{equation}
...
Für ein Mol Gas ($v=1$) folgt aus \cref{eq:innere_energie_f}:
\begin{equation}
    C_V = \frac{\Delta Q}{\Delta T} = \frac{\Delta U}{\Delta T} = \frac{d}{dT} \left(\frac{f}{2} R T\right) = \frac{f}{2} R \mDot
\end{equation}
Dies ist die \textbf{molare Wärmekapazität bei konstantem Volumen}.
...
Es gilt die Beziehung (Mayer-Relation):
\begin{equation}
    C_p = C_V + R = \frac{f}{2}R + R = \frac{f+2}{2} R \mDot
\end{equation}
...
\begin{figure}[htb]
    ...
    Einatomige Gase (wie He) haben nur Translationsfreiheitsgrade ($f=3 \implies C_V \approx 1.5 R$). 
    Zweiatomige Gase (wie $H_2$) haben zusätzlich Rotation ($f=5 \implies C_V \approx 2.5 R$). 
    Bei sehr hohen Temperaturen kommen noch Vibrationen hinzu.}
    \label{fig:cv_gase}
\end{figure}
...
\begin{figure}[htb]
    ...
    \caption{Molare Wärmekapazität fester Stoffe. Bei genügend hoher Temperatur nähern sich viele Stoffe dem klassischen Grenzwert von $3R$ an.}
...
\section{OLD: Wärmemenge und Wärmekapazität}\label{sec: waermemenge_waermekapazitaet}
