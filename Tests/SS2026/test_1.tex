...
\subsection*{Vorgehen}
\begin{enumerate}
    \item Formulieren Sie den Grundsatz des thermischen Gleichgewichts für dieses isolierte System. Wie hängen die abgegebene Wärme des Kupfers und die aufgenommene Wärme des Wassers in diesem abgeschlossenen System zusammen?
    ...
    \item Berechnen Sie aus den beiden Wärmeformeln die Mischtemperatur $T_{\text{Misch}}$, indem Sie das Ergebnis von Punkt 1 verwenden.
\end{enumerate}
...
\quad/1 & \quad/2 & \quad/2.5 & \quad/2.5 & \quad/5 \\
\hline
\end{tabular}
\end{flushright}
...
\subsection*{Vorgehen}
\begin{enumerate}
    \item Zeichnen Sie alle auf das Kind wirkenden Kräfte in die Skizze ein.
    ...
    \item Berechnen Sie mithilfe der Normalkraft die Gleitreibungskraft $F_{R, \text{Gleit}}$ (Formel und Zahlenwert).
    \item Berechnen Sie die resultierende Nettokraft in Bewegungsrichtung (Hangabtrieskraft) und daraus die konstante Beschleunigung des Kindes entlang der Rutsche (Formel und Zahlenwert).
...
\hline
\multicolumn{7}{|l|}{\textbf{\large{Punkte: \quad\quad\quad /14}}}\\
\hline
\textbf{1)} & \textbf{2)} & \textbf{3)} & \textbf{4)} & \textbf{5)} & \textbf{6)} & \textbf{7)} \\
\quad/1.5 & \quad/1 & \quad/2 & \quad/2 & \quad/3 & \quad/3 & \quad/1.5 \\
\hline
\end{tabular}
\end{flushright}
...
\subsection*{Aufgabe}
Berechnen Sie eine Formel für die Eintauchtiefe $h$ des Holzklotzes. Wie tief taucht der Holzklotz in der Angabe in das Wasser ein? Verwenden Sie erst in Unterpunkt 6 Zahlenwerte und geben Sie bei allen anderen Unterpunkten Formeln an.
...
\begin{enumerate}[label=\arabic*.,itemsep=18pt]
    ...
    \begin{itemize}[label={$\square$},itemsep=1pt,topsep=0pt]
        ...
        \item Impuls, Einheit: \si{\kilogram\cdot \meter\per\second}
